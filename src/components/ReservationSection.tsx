...
export const ReservationSection: React.FC = () => {
  ...
  const currentDesc = getCustomText(
    'reservationDescription',
    '전화 및 온라인 예약으로 연락 주시면 실시간 필지 현황과 방문 일정을 즉시 안내해 드립니다.'
  );
  const currentNotes = getCustomText('reservationNotes', '상담 가능 시간: 연중무휴');

  const siteTitle = getCustomText(
    'siteAddressTitle',
    getCustomText('overviewLocation', '현장 위치 (홍보관)')
  );
  const siteText = getCustomText(
    'siteAddressText',
    getCustomText('overviewLocationDetail', '경기도 연천군 장탄리 847-21')
  );
  const siteDesc = getCustomText(
    'siteAddressDesc',
    '전곡역 차량 5분 · 한탄강 조망'
  );

  const officeTitle = getCustomText(
    'officeAddressTitle',
    getCustomText('officeName', '분양 사무실')
  );
  const officeText = getCustomText(
    'officeAddressText',
    getCustomText('officeAddress', '경기도 연천군 전곡역로 67, 1층')
  );
  const officeDesc = getCustomText(
    'officeAddressDesc',
    '전곡역 1번 출구 앞'
  );

  const titleStyle = getCustomTextStyle(
    'reservationTitle',
    'text-4xl',
    'text-3xl',
    'font-black',
  ...
  const descStyle = getCustomTextStyle(
    'reservationDescription',
    'text-base',
    'text-sm',
    'font-normal',
  ...
  const notesStyle = getCustomTextStyle(
    'reservationNotes',
    'text-xs',
    'text-xs',
    'font-semibold',
  ...
  const siteTitleStyle = getCustomTextStyle(
    'siteAddressTitle',
    'text-xs',
    'text-xs',
    'font-black',
  ...
  const siteTextStyle = getCustomTextStyle(
    'siteAddressText',
    'text-sm',
    'text-xs',
    'font-bold',
  ...
  const siteDescStyle = getCustomTextStyle(
    'siteAddressDesc',
    'text-[11px]',
    'text-[11px]',
    'font-normal',
  ...
  const officeTitleStyle = getCustomTextStyle(
    'officeAddressTitle',
    'text-xs',
    'text-xs',
    'font-black',
  ...
  const officeTextStyle = getCustomTextStyle(
    'officeAddressText',
    'text-sm',
    'text-xs',
    'font-bold',
  ...
  const officeDescStyle = getCustomTextStyle(
    'officeAddressDesc',
    'text-[11px]',
    'text-[11px]',
    'font-normal',
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-[#141322] border border-amber-500/30 shadow-2xl space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              ...
              {isDesktop ? (
                ...
                <a
                  ...
                  href={`sms:${config.phone.replace(/[^0-9]/g, '')}?body=${encodeURIComponent(
                    '[골든포레스트 방문 예약 문의] 성함과 희망 방문 일시를 남겨주시면 신속히 연락드리겠습니다.'
                  )}`}
                  onClick={trackKakaoContact}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-white/10">
              ...
              <div className="p-4 rounded-2xl bg-[#100F1E] border border-amber-500/30 space-y-1.5">
                ...
                <div className={`text-white ${siteTextStyle.weight} ${siteTextStyle.size}`}>
                  {siteText}
                </div>

                <div className={`text-slate-400 ${siteDescStyle.weight} ${siteDescStyle.size}`}>
                  {siteDesc}
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-[#100F1E] border border-emerald-500/30 space-y-1.5">
                ...
                <div className={`text-white ${officeTextStyle.weight} ${officeTextStyle.size}`}>
                  {officeText}
                </div>

                <div className={`text-slate-400 ${officeDescStyle.weight} ${officeDescStyle.size}`}>
                  {officeDesc}
                </div>
              </div>
