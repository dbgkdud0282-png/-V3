...
export const ReservationModal: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isDesktop) {
      ...
      const smsUrl = `sms:010-8389-0045?body=${encodeURIComponent(
        smsBody
      )}`;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div
        className={`bg-[#141322] border border-amber-500/30 rounded-3xl w-full relative shadow-2xl animate-scaleUp ${
          isDesktop
            ? 'max-w-xl p-8 space-y-6'
            : 'max-w-lg p-5 space-y-3.5'
        }`}
      >
        {/* Close Button */}
        <button
        ...
        {isSuccess ? (
          <div className="text-center py-6 space-y-4">
            <div
              className={`rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center mx-auto shadow-lg shadow-emerald-500/20 ${
                isDesktop ? 'w-16 h-16' : 'w-14 h-14'
              }`}
            >
              <CheckCircle
                className={isDesktop ? 'w-8 h-8' : 'w-7 h-7'}
              />
            </div>
            ...
            {isDesktop ? (
              <>
                ...
                <p className="text-sm sm:text-base text-slate-300 max-w-md mx-auto leading-relaxed">
                  입력하신 예약 정보가 골든포레스트 관리자 시스템에 실시간으로 등록되었습니다.
                  <br />
              ...
              <>
                ...
                <p className="text-xs sm:text-sm text-slate-300">
                  입력하신 예약 정보가 문자 메시지 본문에 자동으로 입력되었습니다.
                  <br />
          ...
          <>
            <div>
              <span
                className={`font-bold text-amber-400 uppercase tracking-wider block mb-1 ${
                  isDesktop ? 'text-xs' : 'text-[11px]'
                }`}
              >
                {isDesktop
                  ? 'VIP VISIT RESERVATION · ONLINE FORM'
                  : 'GOLDEN FOREST VISIT RESERVATION'}
              </span>
            ...
            <form
              ...
              className={isDesktop ? 'space-y-4' : 'space-y-3'}
            >
              <div>
                <label
                ...
                <input
                  ...
                  className={`w-full rounded-xl bg-[#1B1A2C] border border-white/10 text-white focus:outline-none focus:border-amber-400 transition-colors ${
                    isDesktop
                      ? 'px-4 py-3 text-base'
                      : 'px-3.5 py-2 text-sm sm:text-base'
                  }`}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full rounded-xl bg-[#1B1A2C] border border-white/10 text-white focus:outline-none focus:border-amber-400 transition-colors ${
                    isDesktop
                      ? 'px-4 py-3 text-base placeholder:text-slate-500'
                      : 'px-3.5 py-2 text-sm sm:text-base placeholder:text-slate-500'
                  }`}
                />
              </div>
